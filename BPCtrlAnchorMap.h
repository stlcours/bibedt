...
// $Id: BPCtrlAnchorMap.h,v 1.3 2006/06/06 18:52:18 stievie Exp $
...
public:
  ...
  void Initialize(HWND hWndParent, DWORD dwFlags/*=0*/) {
    ...
    if (dwFlags & ANIF_SIZEGRIP) {
      ...
      m_hWndSizeGrip  = ::CreateWindow(_T("ScrollBar"), _T(""), WS_CHILD | WS_VISIBLE | SBS_SIZEGRIP, 
		  m_rcClient.right-sz1.cx, m_rcClient.bottom-sz1.cy, sz1.cx, sz1.cy, m_hWndParent, NULL, NULL, 0);
    ...
    if (dwFlags & ANIF_CALCSIZE) {
      ...
      for (iCtrl=0; iCtrl < m_nCtrlCount; iCtrl++) {
        ...
        if ((int)m_Ctrl[iCtrl].rect.right > rcMaxBR.right) rcMaxBR.right = (int)m_Ctrl[iCtrl].rect.right;
        if ((int)m_Ctrl[iCtrl].rect.bottom > rcMaxBR.bottom) rcMaxBR.bottom = (int)m_Ctrl[iCtrl].rect.bottom;
