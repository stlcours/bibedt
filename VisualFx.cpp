...
//  pDC->SelectObject(&m_PenWhite2);
  ...
  pDC->MoveTo(x,y+TABSEL_HEIGHT);
  pDC->LineTo(x,y+1);
  pDC->MoveTo(x+1,y+1);
  pDC->LineTo(x+textSize.cx+TAB_SPACE-3, y+1);

  // right border, dark gray, double line
//  pDC->SelectObject(&m_PenDGray2);
  pDC->SelectObject(&m_PenDGray);
  pDC->MoveTo(x+textSize.cx+TAB_SPACE-3, y+2);
  pDC->LineTo(x+textSize.cx+TAB_SPACE-3, y+TABSEL_HEIGHT-1);

  pDC->SelectObject(&m_PenBlack);
  pDC->MoveTo(x+textSize.cx+TAB_SPACE-2, y+3);
  pDC->LineTo(x+textSize.cx+TAB_SPACE-2, y+TABSEL_HEIGHT);
...
//                  &m_BrushBlack);
  ...
  pDC->SelectObject(&m_PenDGray);
  pDC->MoveTo(x+textSize.cx+TAB_SPACE-3, y+2);
  pDC->LineTo(x+textSize.cx+TAB_SPACE-3, y+TAB_HEIGHT);
  // black border
  pDC->SelectObject(&m_PenBlack);
  pDC->MoveTo(x+textSize.cx+TAB_SPACE-2, y+3);
  pDC->LineTo(x+textSize.cx+TAB_SPACE-2, y+TAB_HEIGHT);

  pDC->FillRect(&CRect(CPoint(x+2,y+2), CSize(textSize.cx+TAB_SPACE-5, TAB_HEIGHT-3)), 
                &m_BrushLGray);
...
TVisualFramework::~TVisualFramework()
{
  if (m_ObjectMap.size() && m_ObjectList.size()) {
    TRACE0(">>> TVisualFramework::Destroy() called in TVisualFramework destructor\n");
    TRACE0(">>>   It must be called in CFrameWnd derived class OnDestroy() message handler\n");
    Destroy();
  }
}
...
BOOL TVisualFramework::Create(CWnd *pWnd)
{
  ...
  for (it = m_ObjectList.begin(); it != m_ObjectList.end(); it++) {
    ...
    if (rc == FALSE) {
      TRACE0("Create visual object failed!\n");
      return FALSE;
    }
  }
